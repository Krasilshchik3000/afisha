#!/usr/bin/env python3
"""
URL-кодирует имена файлов обложек в index.html
"""
import csv
import os
import glob
...
def generate_cover_html(name, date, pdf_link):
    """Генерирует HTML для одной обложки с URL-кодированием"""
    year = date.split('-')[0] if date else "unknown"
    
    local_file = find_local_cover(name, year)
    
    if local_file:
        # URL-кодируем имя файла для корректной работы в браузере
        cover_src_encoded = quote(f"covers_medium/{local_file}", safe='/')
        cover_src = f"covers_medium/{local_file}"
    else:
        cover_src = cover_src_encoded = "fav.jpeg"
