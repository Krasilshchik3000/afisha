#!/usr/bin/env python3
"""
Исправляет HTML с реальными именами файлов обложек
"""
import csv
import os
import re
...
def sanitize_for_search(name):
    """Очищает имя для поиска файла"""
    # Заменяем специальные символы
    clean = name.replace('№', 'N').replace('–', '-')
...
def find_actual_cover_file(name, date):
    ...
    patterns = [
        f"covers_medium/cover_{year}_{sanitize_for_search(name)}*",
        f"covers_medium/cover_{year}_{name.replace('№', 'N').replace(' ', '_').replace('(', '').replace(')', '').replace('–', '-')}*",
    ]
    
    for pattern in patterns:
        files = glob.glob(pattern)
        if files:
            # Берем файл с наименьшим индексом
            files.sort()
...
def generate_cover_html(name, date, pdf_link):
    ...
    if not img_file:
        # Если не нашли файл, используем fallback
        img_file = "fav.jpeg"
